...
{
  std::cout << "Looking for group" << std::endl;
...
{
  std::cout << "see new friends" << std::endl;
  ...
  if (thisSwarmComp.getGroupHealth(BehaviorTree::sceneHandler->getScene()) >
      thisSwarmComp.LOW_HEALTH)
    {
      for (auto& f : thisSwarmComp.group->members)
        {
          SwarmComponent& curSwarmComp = BehaviorTree::sceneHandler->getScene()
...
/* BTStatus SwarmBT::jumpTowardsPlayer(Entity entityID)
{
    ...
    while(!canSeePlayer) 
    {
        ...
        {        
            ...
            if (getPlayerID(sceneHandler) != rp.entity)
            {
            ...
            canSeePlayer = rp.entity == getPlayerID(sceneHandler);
        }
...
BTStatus SwarmBT::jumpTowardsPlayer(Entity entityID)
{
  ...
  //float playerLen = playerCollider.radius * 2;
  ...
  glm::vec3 entityTargetPos = entityTransform.position;
  float distEntityTargetPosToPlayer =
      glm::length(newPlayerPos - entityTargetPos);
  glm::vec3 dirEntityTargetPosToPlayer =
...
BTStatus SwarmBT::attack(Entity entityID)
{
  ...
  AiCombat& combat =
      BehaviorTree::sceneHandler->getScene()->getComponent<AiCombat>(entityID);
  Rigidbody& rigidbody =
      BehaviorTree::sceneHandler->getScene()->getComponent<Rigidbody>(entityID);
  ...
  if (combat.timer > 0.0f)
    ...
    {
      Ray downRay{thisTransform.position, glm::vec3(0.f, -1.f, 0.1f)};
...
BTStatus SwarmBT::die(Entity entityID)
{
  ...
  Combat& playerCombat =
      sceneHandler->getScene()->getComponent<Combat>(getPlayerID(sceneHandler));
