...
BTStatus SwarmBT::hasFriends(Entity entityID)
{
  ...
  SwarmGroup* groupPtr = BehaviorTree::sceneHandler->getScene()->getComponent<SwarmComponent>(entityID).group;
...
BTStatus SwarmBT::JoinGroup(Entity entityID)
{
  ...
  BehaviorTree::sceneHandler->getScene()->getSceneReg()
  ...
  if (rand() % 2 == 0)
    {
      thisTransform.rotation.y +=
          100 * thisSwarmComp.escapeAnimSpeed * Time::getDT();
    }
  else
    {
      thisTransform.rotation.y -=
...
BTStatus SwarmBT::jumpTowardsPlayer(Entity entityID)
{
  ...
  Collider& playerCollider = BehaviorTree::sceneHandler->getScene()->getComponent<Collider>(getPlayerID(sceneHandler,entityID));
  Transform& entityTransform = BehaviorTree::sceneHandler->getScene()->getComponent<Transform>(entityID);
  Transform& playerTransform = BehaviorTree::sceneHandler->getScene()->getComponent<Transform>(getPlayerID(sceneHandler, entityID));
  SwarmComponent& swarmComp = BehaviorTree::sceneHandler->getScene()->getComponent<SwarmComponent>(entityID);
  Rigidbody& rigidbody = BehaviorTree::sceneHandler->getScene()->getComponent<Rigidbody>(entityID);

  ((NetworkScene*)BehaviorTree::sceneHandler->getScene())
      ->addEvent(
          {(int)GameEvents::Draw_Debug_Line},
  ...
  newPlayerPos.y += 3.5f;
  //TODO : code that cannot be in here on server
  ...
  while (!canSeePlayer)
    {
      ...
      dirEntityTargetPosToPlayer =
          glm::normalize(newPlayerPos - entityTargetPos);
      ...
      RayPayload rp = BehaviorTree::sceneHandler->getPhysicsEngine()->raycast(
          rayToPlayer, distEntityTargetPosToPlayer + 10.f
      );
...
BTStatus SwarmBT::die(Entity entityID)
{
  ...
  static const int perkSpawnChance = 2;
  ...
  {
      //single player
      Combat& playerCombat = sceneHandler->getScene()->getComponent<Combat>(getPlayerID(sceneHandler, entityID));
      ...
      if (spawnLoot < 2)
        {
          ...
          sceneHandler->getScene()->setComponent<MeshComponent>(
              perkEnt, SwarmBT::perkMeshes[perkType]
          );
          Transform& perkTrans =
              sceneHandler->getScene()->getComponent<Transform>(perkEnt);
          Transform& swarmTrans =
              sceneHandler->getScene()->getComponent<Transform>(entityID);
          perkTrans.position = swarmTrans.position;
          perkTrans.scale = glm::vec3(2.f, 2.f, 2.f);
          sceneHandler->getScene()->setComponent<Collider>(
              perkEnt, Collider::createSphere(2.f, glm::vec3(0.f, 0.f, 0.f), true)
          );
          sceneHandler->getScene()->setComponent<Rigidbody>(perkEnt);
          Rigidbody& perkRb =
              sceneHandler->getScene()->getComponent<Rigidbody>(perkEnt);
          glm::vec3 spawnDir =
          ...
          sceneHandler->getScene()->setComponent<PointLight>(
              perkEnt, glm::vec3(5.f, 7.f, 9.f)
          );
        }
      else if (spawnLoot == 2)
        {
          ...
          sceneHandler->getScene()->setComponent<MeshComponent>(
              abilityEnt, SwarmBT::abilityMeshes[abilityType]
          );
          Transform& abilityTrans =
              sceneHandler->getScene()->getComponent<Transform>(abilityEnt);
          Transform& swarmTrans =
              sceneHandler->getScene()->getComponent<Transform>(entityID);
          ...
          sceneHandler->getScene()->setComponent<Collider>(
              abilityEnt,
              Collider::createSphere(4.f, glm::vec3(0.f, 0.f, 0.f), true)
          );
          sceneHandler->getScene()->setComponent<Rigidbody>(abilityEnt);
          Rigidbody& abilityRb =
              sceneHandler->getScene()->getComponent<Rigidbody>(abilityEnt);
          glm::vec3 spawnDir =
              glm::vec3((rand() % 201) * 0.01f - 1, 1, (rand() % 200) * 0.01f - 1);
          ...
          sceneHandler->getScene()->setComponent<PointLight>(
              abilityEnt, glm::vec3(7.f, 9.f, 5.f)
          );
        }
  }
