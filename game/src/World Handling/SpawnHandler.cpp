...
)
{
    ...
    Transform& transform =
        currScene->getComponent<Transform>(this->tankIDs[tankIdx]);
    ...
    TankComponent& tankComp =
        currScene->getComponent<TankComponent>(this->tankIDs[tankIdx]);
    ...
    if (elite)
        {
            if (!tankComp.isElite)
                {
                    tankComp.applyEliteStats(
                        defaultEliteComp, currScene, this->tankIDs[tankIdx]
                    );
                }
        }
    else
        {
            if (tankComp.isElite)
                {
                    tankComp.removeEliteStats(
                        currScene, this->tankIDs[tankIdx]
                    );
                }
        }
    ...
    if (tileInfos.size() == 2)
        {
            ...
            const int alterID =
                this->lichObjects[this->lichIDs[lichIdx]].alterID;
            const int graveID =
                this->lichObjects[this->lichIDs[lichIdx]].graveID;
            ...
            Transform& transform =
                currScene->getComponent<Transform>(this->lichIDs[lichIdx]);
            ...
            LichComponent& lichComp =
                currScene->getComponent<LichComponent>(this->lichIDs[lichIdx]);
            //Setas elite
            AiEliteComponent defaultEliteComp;
            ...
            if (elite)
                {
                    if (!lichComp.isElite)
                        {
                            lichComp.applyEliteStats(
                                defaultEliteComp,
                                currScene,
                                this->lichIDs[lichIdx]
                            );
                        }
                }
            else
                {
                    if (lichComp.isElite)
                        {
                            lichComp.removeEliteStats(
                                currScene, this->lichIDs[lichIdx]
                            );
                        }
                }
            //Reset
            ...
            Transform& alterTransform =
                currScene->getComponent<Transform>(alterID);
            alterTransform.position = alterPos->getPos();

            // Place Grave
            Transform& graveTransform =
                currScene->getComponent<Transform>(graveID);
            graveTransform.position =
                this->tilePicker.getRandomFreeTileFarAwayFrom(alterPos)->getPos(
                );
            ...
            if (dynamic_cast<NetworkScene*>(currScene) != nullptr)
                {
                    ((NetworkScene*)currScene)
                        ->addEvent(
                            {(int)GameEvent::ACTIVATE, this->lichIDs[lichIdx]}
                        );
                    ((NetworkScene*)currScene)
                        ->addEvent({(int)GameEvent::ACTIVATE, alterID});
                    ((NetworkScene*)currScene)
                        ->addEvent({(int)GameEvent::ACTIVATE, graveID});
                    ((NetworkScene*)currScene)
                        ->addEvent(
                            {(int)GameEvent::SET_POS_OBJECT, alterID},
                            {alterTransform.position.x,
                             alterTransform.position.y,
                             alterTransform.position.z}
                        );
                    ((NetworkScene*)currScene)
                        ->addEvent(
                            {(int)GameEvent::SET_POS_OBJECT, graveID},
                            {graveTransform.position.x,
...
void SpawnHandler::spawnSwarm(int swarmIdx, const glm::vec3& pos, bool elite)
{
    ...
    Transform& transform =
        currScene->getComponent<Transform>(this->swarmIDs[swarmIdx]);
    ...
    SwarmComponent& swarmComp =
        currScene->getComponent<SwarmComponent>(this->swarmIDs[swarmIdx]);
    ...
    if (elite)
        {
            if (!swarmComp.isElite)
                {
                    swarmComp.applyEliteStats(
                        defaultEliteComp, currScene, this->swarmIDs[swarmIdx]
                    );
                }
        }
    else
        {
            if (swarmComp.isElite)
                {
                    swarmComp.removeEliteStats(
                        currScene, this->swarmIDs[swarmIdx]
                    );
                }
        }
    ...
    transform.rotation.y = lookAtY(
        transform.position, transform.position + genRandomDir({1.f, 0.f, 1.f})
    );
...
void SpawnHandler::initTanks()
{
    for (auto t : tankIDs)
        {
            TankComponent& tankComp =
                this->currScene->getComponent<TankComponent>(t);
...
void SpawnHandler::killAllEnemiesOutsideRoom()
{
    ...
    this->sceneHandler->getScene()
        ->getSceneReg()
        .view<FSMAgentComponent, Transform>(entt::exclude<Inactive>)
        .each(
            [&](const auto& entity, FSMAgentComponent, Transform& transform)
            {
                bool enemyUnderFloor = transform.position.y < 0;
                bool enemyOverRoof =
                    transform.position.y > RoomHandler::BORDER_COLLIDER_HEIGHT;
                ...
                Ray rayToMiddle{
                    tempPos,
                    safeNormalize(this->roomHandler->getRoomPos() - tempPos)};

                RayPayload rp = this->sceneHandler->getPhysicsEngine()->raycast(
                    rayToMiddle,
                    glm::length(this->roomHandler->getRoomPos() - tempPos)
                );

                if (rp.hit)
                    {
                        if (this->sceneHandler->getScene()
                                ->hasComponents<EdgeTile>(rp.entity))
                            {
                ...
                if (enemyUnderFloor || enemyOverRoof || enemyOutsideRoom)
                    {
                        ...
                        if (this->sceneHandler->getScene()
                                ->hasComponents<SwarmComponent>(id))
                            {
                                auto& comp =
                                    this->sceneHandler->getScene()
                                        ->getComponent<SwarmComponent>(id);
                                comp.life = 0;
#ifdef _CONSOLE
                                Log::write(
                                    "Killing Swarm[" + std::to_string(id) +
                                    "] enemies outside room..."
                                );
#endif
                            }
                        else if (this->sceneHandler->getScene()
                                     ->hasComponents<LichComponent>(id))
                            {
                                auto& comp =
                                    this->sceneHandler->getScene()
                                        ->getComponent<LichComponent>(id);
                                comp.life = 0;
#ifdef _CONSOLE
                                Log::write(
                                    "Killing Lich[" + std::to_string(id) +
                                    "] enemies outside room..."
                                );
#endif
                            }
                        else if (this->sceneHandler->getScene()
                                     ->hasComponents<TankComponent>(id))
                            {
                                auto& comp =
                                    this->sceneHandler->getScene()
                                        ->getComponent<TankComponent>(id);
                                comp.life = 0;
#ifdef _CONSOLE
                                Log::write(
                                    "Killing Tank[" + std::to_string(id) +
                                    "] enemies outside room..."
                                );
#endif
                            }
...
void SpawnHandler::createTank()
{
            ...
            if (tank == -1)
                {
                    ...
                    this->resourceManager->mapAnimations(
                        tank,
                        {
                            "Walk",
                            "Charge",
                    ...
                    this->resourceManager->createAnimationSlot(
                        tank, "LowerBody", "Character1_Hips"
                    );
                    this->resourceManager->createAnimationSlot(
                        tank, "UpperBody", "Character1_Spine"
                    );
                }
            this->currScene->setComponent<MeshComponent>(
                this->tankIDs.back(), tank
            );
            this->currScene->setComponent<AnimationComponent>(
                this->tankIDs.back()
            );
        }
    else
    ...
    rb.mass = 100.0f;
    Transform& transform =
        this->currScene->getComponent<Transform>(this->tankIDs.back());
    this->currScene->setComponent<Collider>(
        this->tankIDs.back(),
        Collider::createSphere(
            TankComponent::colliderRadius, glm::vec3(0.0f, 19.5f, 0.0f)
        )
    );
    this->aiHandler->createAIEntity(this->tankIDs.back(), "tankFSM");
    TankComponent& tankComp =
        this->currScene->getComponent<TankComponent>(this->tankIDs.back());
    tankComp.origScaleY = transform.scale.y;
    ...
    if (netScene != nullptr)
        {
            netScene->addEvent(
                {(int)GameEvent::INACTIVATE, this->tankIDs.back()}
            );
        ...
        {
            static int tankHump =
                this->resourceManager->addMesh("assets/models/hump.obj");
            for (int i = 0; i < nrOfHumps; i++)
                {
...
void SpawnHandler::createLich()
{
    ...
    Rigidbody& rb =
        this->currScene->getComponent<Rigidbody>(this->lichIDs.back());
    rb.rotFactor = glm::vec3(0.0f, 0.0f, 0.0f);
    rb.gravityMult = 5.0f;
    rb.friction = 3.0f;
    rb.mass = 10.0f;
    Transform& transform =
        this->currScene->getComponent<Transform>(this->lichIDs.back());
    transform.scale = glm::vec3(1.5f);
    this->currScene->setComponent<Collider>(
        this->lichIDs.back(),
        Collider::createCapsule(
            LichComponent::colliderRadius,
            LichComponent::colliderHeight,
            glm::vec3(0.0f, LichComponent::colliderYOffset, 0.0f)
        )
    );
    this->aiHandler->createAIEntity(this->lichIDs.back(), "lichFSM");
    LichComponent& lichComp =
        this->currScene->getComponent<LichComponent>(this->lichIDs.back());
    lichComp.origScaleY = transform.scale.y;
    ...
    //Create Grave
    auto& graveID = this->lichObjects[this->lichIDs.back()].graveID =
        this->currScene->createEntity();
    this->currScene->setComponent<Collider>(
        graveID,
        Collider::createBox(
            glm::vec3{
    ...
    this->currScene->getComponent<LichComponent>(this->lichIDs.back()).graveID =
        graveID;
    this->currScene->setInactive(graveID);

    //Create Alter
    auto& alterID = this->lichObjects[this->lichIDs.back()].alterID =
        this->currScene->createEntity();
    this->currScene->setComponent<Collider>(
        alterID,
        Collider::createBox(
    ...
    this->currScene->getComponent<LichComponent>(this->lichIDs.back()).alterID =
        alterID;
    ...
    if (netScene == nullptr)
        {
            static int grave =
                this->resourceManager->addMesh("assets/models/grave.obj");
            static int alter =
                this->resourceManager->addMesh("assets/models/alter.obj");
            static int fireOrb_mesh =
                this->resourceManager->addMesh("assets/models/fire_orb.obj");
            static int lightOrb_mesh =
                this->resourceManager->addMesh("assets/models/light_orb.obj");
            static int iceOrb_mesh =
                this->resourceManager->addMesh("assets/models/ice_orb.obj");
            ...
            for (size_t i = 0; i < LichComponent::NR_FIRE_ORBS; i++)
                {
                    ...
                    this->currScene->setComponent<Collider>(
                        lichComp.fireOrbs[i],
                        Collider::createSphere(
                            LichComponent::orbRadius, glm::vec3(0.0f), true
                        )
                    );
                    this->currScene->setComponent<Orb>(lichComp.fireOrbs[i]);
                    this->currScene->setInactive(lichComp.fireOrbs[i]);
                    this->currScene->setComponent<Rigidbody>(
                        lichComp.fireOrbs[i]
                    );
                    Rigidbody& rb = this->currScene->getComponent<Rigidbody>(
                        lichComp.fireOrbs[i]
                    );
                    rb.rotFactor = glm::vec3(0.0f, 0.0f, 0.0f);
                    rb.gravityMult = 0.0f;
            ...
            for (size_t i = 0; i < LichComponent::NR_FIRE_ORBS; i++)
                {
                    this->currScene->setComponent<MeshComponent>(
                        lichComp.fireOrbs[i], fireOrb_mesh
                    );
                }
            for (size_t i = 0; i < LichComponent::NR_ICE_ORBS; i++)
                {
                    this->currScene->setComponent<MeshComponent>(
                        lichComp.iceOrbs[i], iceOrb_mesh
                    );
                }
            for (size_t i = 0; i < LichComponent::NR_LIGHT_ORBS; i++)
                {
                    this->currScene->setComponent<MeshComponent>(
                        lichComp.lightOrbs[i], lightOrb_mesh
                    );
                }
        }
    else
        {
            ...
            netScene->addEvent(
                {(int)GameEvent::SPAWN_OBJECT,
                 (int)graveID,
                 (int)ObjectTypes::LICH_GRAVE},
                {0.f, 0.f, 0.f, 0.f, 0.f, 0.f, 1.f, 1.f, 1.f}
            );
            netScene->addEvent(
                {(int)GameEvent::SPAWN_OBJECT,
                 (int)alterID,
                 (int)ObjectTypes::LICH_ALTER},
                {0.f, 0.f, 0.f, 0.f, 0.f, 0.f, 1.f, 1.f, 1.f}
            );
            ...
            for (size_t i = 0; i < LichComponent::NR_FIRE_ORBS; i++)
                {
                    ...
                    netScene->addEvent(
                        {(int)GameEvent::SPAWN_ORB,
                         lichComp.fireOrbs[i],
                         (int)ATTACK_STRATEGY::FIRE}
                    );
                }
            for (size_t i = 0; i < LichComponent::NR_ICE_ORBS; i++)
                {
    ...
    for (size_t i = 0; i < SpawnHandler::NR_BLOBS_IN_GROUP; i++)
        {
            if (netScene == nullptr)
                {
                    ...
                    static int swarm = this->resourceManager->addMesh(
                        "assets/models/Swarm_RotTest.obj"
                    );
                    this->currScene->setComponent<MeshComponent>(
                        this->swarmIDs.back(), swarm
                    );
                }
            else
                {
            ...
            this->currScene->setComponent<Collider>(
                this->swarmIDs.back(),
                Collider::createSphere(SwarmComponent::colliderRadius)
            );
            this->currScene->setComponent<Rigidbody>(this->swarmIDs.back());
            Rigidbody& rb =
                this->currScene->getComponent<Rigidbody>(this->swarmIDs.back());
            rb.rotFactor = glm::vec3(0.0f, 0.0f, 0.0f);
            rb.gravityMult = 5.0f;
            rb.friction = 1.5f;
            ...
            this->sceneHandler->getScene()
                ->getComponent<SwarmComponent>(this->swarmIDs.back())
                .group = this->swarmGroups.back();
            Transform& transform =
                this->currScene->getComponent<Transform>(this->swarmIDs.back());
            SwarmComponent& swarmComp =
                this->currScene->getComponent<SwarmComponent>(
                    this->swarmIDs.back()
                );
            swarmComp.life = 0;
            swarmComp.origScaleY = transform.scale.y;
            ...
            if (netScene != nullptr)
                {
                    netScene->addEvent(
                        {(int)GameEvent::INACTIVATE, this->swarmIDs.back()}
                    );
                }
        }
}
                    ...
                    if (ImGui::Button("Kill all"))
                        {
                            this->sceneHandler->getScene()
                                ->getSceneReg()
                                .view<SwarmComponent>(entt::exclude<Inactive>)
                                .each([&](SwarmComponent& swarmComp)
                                      { swarmComp.life = 0; });
                            this->sceneHandler->getScene()
                                ->getSceneReg()
                                .view<LichComponent>(entt::exclude<Inactive>)
                                .each([&](LichComponent& lichComp)
                                      { lichComp.life = 0; });
                            this->sceneHandler->getScene()
                                ->getSceneReg()
                                .view<TankComponent>(entt::exclude<Inactive>)
                                .each([&](TankComponent& tankComp)
                                      { tankComp.life = 0; });
                        }
                    ImGui::Separator();
                    ImGui::Text(
                        "# Enimies Total : %d", (int)this->nrOfEnemiesPerRoom
                    );
...
ImguiLambda SpawnHandler::TankImgui()
{
    ...
    return [&](FSM* fsm, uint32_t entityId) -> void
    {
        ...
        auto entityImguiWindow = [&](TankFSM* tankFsm,
                                     uint32_t entityId) -> void
        {
            auto& entityTankComponent =
                this->sceneHandler->getScene()->getComponent<TankComponent>(
                    entityId
                );
            auto& entiyFSMAgentComp =
                this->sceneHandler->getScene()->getComponent<FSMAgentComponent>(
                    entityId
                );
            auto& entityRigidBody =
                this->sceneHandler->getScene()->getComponent<Rigidbody>(entityId
                );
            const glm::vec3& tankPos = this->sceneHandler->getScene()
                                           ->getComponent<Transform>(entityId)
                                           .position;
            int& health = entityTankComponent.life;
            std::string fis =
                "Friends in sight: " +
                std::to_string(entityTankComponent.friendsInSight.size());
            std::string af =
                "All friends alive: " +
                std::to_string(entityTankComponent.allFriends.size());
        ...
        if (showEntityId)
            {
                ...
                auto screenPos = this->currScene->getMainCamera()->projection *
                                 this->currScene->getMainCamera()->view *
                                 entityPos;
...
ImguiLambda SpawnHandler::LichImgui()
{
    ...
    return [&](FSM* fsm, uint32_t entityId) -> void
    {
        ...
        auto& lichComponent =
            this->sceneHandler->getScene()->getComponent<LichComponent>(entityId
            );
        auto& entiyFSMAgentComp =
            this->sceneHandler->getScene()->getComponent<FSMAgentComponent>(
                entityId
        ...
        const glm::vec3& lichPos = this->sceneHandler->getScene()
                                       ->getComponent<Transform>(entityId)
                                       .position;
        ImGui::Text(
            "pos: (%d, %d, %d)", (int)lichPos.x, (int)lichPos.y, (int)lichPos.z
        );
        ImGui::Separator();
...
ImguiLambda SpawnHandler::SwarmImgui()
{
    return [&](FSM* fsm, uint32_t entityId) -> void
    {
        ...
        auto entityImguiWindow = [&](SwarmFSM* swarmFsm,
                                     uint32_t entityId) -> void
        {
            auto& entitySwarmComponent =
                this->sceneHandler->getScene()->getComponent<SwarmComponent>(
                    entityId
                );
            auto& entiyFSMAgentComp =
                this->sceneHandler->getScene()->getComponent<FSMAgentComponent>(
            ...
            auto& entityTransform =
                this->sceneHandler->getScene()->getComponent<Transform>(entityId
                );
            ...
            float& gravity = entityRigidBody.gravityMult;
            std::string& status = entiyFSMAgentComp.currentNode->status;
            const glm::vec3& blobPos = this->sceneHandler->getScene()
                                           ->getComponent<Transform>(entityId)
                                           .position;
        ...
        if (showEntityId)
            {
                ...
                // Show all entity ID over entitties
                glm::vec3 entityPos3 = this->sceneHandler->getScene()
                                           ->getComponent<Transform>(entityId)
                                           .position;
                ...
                //Scene::getUIRenderer()->renderString(std::to_string(entityId), realScreenPos.x, realScreenPos.y, 20, 20);
                uiRenderer->renderString(
                    std::to_string(entityId), entityPos3, glm::vec2(20, 20)
                );
            }
        ...
        std::string groupName =
            "GroupMembers[" + std::to_string(entitySwarmComponent.group->myId) +
            "]";
