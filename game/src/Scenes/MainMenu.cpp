...
void MainMenu::update()
{
	...
	case Menu:
		if (this->getComponent<UIArea>(playButton).isClicking())
        {
            ...
            this->getNetworkHandler()->createServer(new_ NetworkLobbyScene());
			this->getSceneHandler()->setScene(new_ logInScene());
		}
...
#ifdef WIN32
	case LevelEdit:
		this->switchScene(new_ LevelEditor(), "scripts/levelEditor.lua");
